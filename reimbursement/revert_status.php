<?php
/**
 * Revert workflow status of a reimbursement request to a prior stage.
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    pop('Invalid request method.', '/reimbursement/list.php', POP_DEFAULT_DELAY_MS, 'error');
    exit;
}
...
// Fetch request
$stmt = $pdo->prepare("SELECT * FROM procurement_requests WHERE request_id = ? AND request_type = 'REIMBURSEMENT'");
$stmt->execute([$id]);
$request = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$request) {
    modalPop('Not Found', 'Reimbursement request not found.', '/reimbursement/list.php', 'error');
    exit;
}
...
try {
    ...
    if (class_exists('NotificationService')) {
        NotificationService::createNotification(
            ...
            [
                'title'       => "Reimbursement Returned: {$request['request_number']}",
