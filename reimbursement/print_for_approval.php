<?php
/**
 * Print Reimbursement Invoice for Approval
 * 
 * Generates a clean PDF document that Finance Officers can print, sign, and return
 * This is reimbursement-specific with invoice details, approval routing, and signatures
 */
...
require_once $_SERVER['DOCUMENT_ROOT'].'/services/RequestPrintService.php';
...
if (!$request) {
    ...
    exit('Reimbursement request not found or invalid request type');
}
...
$isAuthorizedRole = in_array($_SESSION['role_name'] ?? '', 
    ['Finance Officer', 'Director HRM&A', 'Admin', 'SuperAdmin', 'Procurement Officer']);

if (!$isRequestor && !$isAuthorizedRole) {
    http_response_code(403);
    logAudit($pdo, 'procurement_requests', $request_id, 'ACCESS_DENIED',
        'User ' . ($_SESSION['full_name'] ?? 'Unknown') . ' attempted unauthorized access to reimbursement request print');
    exit('You do not have permission to print this reimbursement request');
}
...
// Fetch reimbursement-specific invoice details
try {
    $invoiceStmt = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $invoiceStmt->execute([$request_id]);
...
if ($invoice) {
    try {
        $attachStmt = $pdo->prepare("
            SELECT COUNT(*) as count FROM reimbursement_invoice_attachments
            WHERE reimb_invoice_id = ? AND is_deleted = 0
        ");
        $attachStmt->execute([$invoice['reimb_invoice_id']]);
        $attachmentCount = $attachStmt->fetch(PDO::FETCH_ASSOC)['count'];
    } catch (Exception $e) {
        $attachmentCount = 0;
    }
}
...
$html .= '<h2 style="margin: 10px 0 0 0; font-size: 18px; font-weight: normal;">REIMBURSEMENT INVOICE FOR APPROVAL</h2>';
$html .= '</div>';
...
if ($invoice) {
    $html .= '<table style="width: 100%; margin-bottom: 20px; font-size: 12px; border-collapse: collapse;">';
    $html .= '<tr style="background-color: #e8e8e8;">';
    $html .= '<td colspan="2" style="padding: 8px; font-weight: bold; border: 1px solid #999;">INVOICE DETAILS</td>';
    $html .= '</tr>';
    $html .= '<tr>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc; width: 50%; font-weight: bold;">Invoice ID:</td>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc; width: 50%;">'. htmlspecialchars($invoice['invoice_id'] ?? 'N/A') .'</td>';
    $html .= '</tr>';
    $html .= '<tr>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc; font-weight: bold;">Invoice Amount:</td>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc;">'. number_format($invoice['invoice_amount'] ?? 0, 2) .' '. htmlspecialchars($request['currency'] ?? 'USD') .'</td>';
    $html .= '</tr>';
    $html .= '<tr>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc; font-weight: bold;">Invoice Stage:</td>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc;">'. htmlspecialchars($invoice['invoice_stage'] ?? 'N/A') .'</td>';
    $html .= '</tr>';
    $html .= '<tr>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc; font-weight: bold;">Submitted By:</td>';
    $html .= '<td style="padding: 8px; border: 1px solid #ccc;">'. htmlspecialchars($invoice['submitted_by_name'] ?? 'N/A') .' on '. htmlspecialchars(substr($invoice['submitted_date'], 0, 10) ?? '') .'</td>';
    $html .= '</tr>';
    $html .= '<tr>';
    $html .= '<td colspan="2" style="padding: 8px; border: 1px solid #ccc;">';
    $html .= '<strong>Supporting Documents Attached:</strong> '. $attachmentCount .'</td>';
    $html .= '</tr>';
    $html .= '</table>';
}
...
$html .= '<table style="width: 100%; margin-bottom: 20px; font-size: 12px; border-collapse: collapse;">';
$html .= '<tr style="background-color: #e8e8e8;">';
$html .= '<td colspan="2" style="padding: 8px; font-weight: bold; border: 1px solid #999;">REQUIRED APPROVALS</td>';
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td style="padding: 8px; border: 1px solid #ccc; width: 40%; font-weight: bold;">1. Finance Officer Verification</td>';
$html .= '<td style="padding: 8px; border: 1px solid #ccc;">Verify invoice authenticity and amounts</td>';
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td style="padding: 8px; border: 1px solid #ccc; font-weight: bold;">2. Director Approval</td>';
$html .= '<td style="padding: 8px; border: 1px solid #ccc;">Final authorization for reimbursement</td>';
$html .= '</tr>';
$html .= '</table>';
...
$html .= '<table style="width: 100%; margin-bottom: 20px; font-size: 12px; border-collapse: collapse;">';
$html .= '<tr style="background-color: #e8e8e8;">';
$html .= '<td colspan="2" style="padding: 8px; font-weight: bold; border: 1px solid #999;">AUTHORIZATION SIGNATURES</td>';
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td style="padding: 15px; border: 1px solid #ccc; width: 50%; text-align: center;">';
$html .= '<strong>Finance Officer Verification</strong><br/><br/><br/>';
$html .= '___________________________<br/>';
$html .= 'Signature & Date';
$html .= '</td>';
$html .= '<td style="padding: 15px; border: 1px solid #ccc; width: 50%; text-align: center;">';
$html .= '<strong>Director Approval</strong><br/><br/><br/>';
$html .= '___________________________<br/>';
$html .= 'Signature & Date';
$html .= '</td>';
$html .= '</tr>';
...
$html .= '<p style="margin: 5px 0;">For questions about this reimbursement request, please contact the Procurement Office.</p>';
$html .= '</div>';
...
try {
    ...
    logAudit($pdo, 'request_documents', null, 'PRINT_EVENT',
        'Reimbursement request ' . htmlspecialchars($request['request_number']) . ' printed for approval by ' . ($_SESSION['full_name'] ?? 'Unknown'));
    
    // Output PDF
    header('Content-Type: application/pdf');
    header('Content-Disposition: attachment; filename="REIMB_' . htmlspecialchars($request['request_number']) . '_' . date('Ymd_His') . '.pdf"');
    echo $dompdf->output();
