...
if ($rfq_id) {
    ...
    if ($action === 'approve') {
        ...
        /* Notify about commitment stage approval */
        require_once $_SERVER['DOCUMENT_ROOT']."/config/notifications.php";
        if ($remainingCount == 0) {
            notifyCommitmentAction($commitment['request_id'], $commitment['commitment_number'], 'APPROVED', 'All commitment approval stages complete. Ready for PO creation.');
        } else {
            notifyCommitmentAction($commitment['request_id'], $commitment['commitment_number'], 'STAGE_APPROVED', 'Commitment stage approved by ' . $current_role . '. Next approval stage pending.');
        }
    ...
    elseif ($action === 'reject') {
        ...
        /* Notify requestor of commitment rejection */
        require_once $_SERVER['DOCUMENT_ROOT']."/config/notifications.php";
        notifyCommitmentAction($commitment['request_id'], $commitment['commitment_number'], 'DECLINED', 'Commitment stage rejected by ' . $current_role . '. Reason: ' . $reason);
