<?php
/**
 * Upload Signed Petty Cash Reconciliation Approval Form
 * 
 * Handles signed reconciliation form uploads
...
requirePostRequest('/petty_cash/list.php');
requireCsrfToken('/petty_cash/list.php');

$request_id = (int)($_POST['request_id'] ?? 0);
if ($request_id <= 0) {
    pop("Invalid request.", "/petty_cash/list.php", 2500, "error");
    exit;
}
...
try {
    ...
    pop("Error fetching request: " . extractDbMessage($e), "/petty_cash/list.php", 2500, "error");
    exit;
}

if (!$request) {
    pop("Petty cash request not found.", "/petty_cash/list.php", 2500, "error");
    exit;
}
...
try {
    ...
    if (!$authCheck['authorized']) {
        pop($authCheck['reason'], "/petty_cash/list.php", 2500, "error");
        exit;
    }
    ...
    if (!$workflowCheck['allowed']) {
        pop($workflowCheck['reason'], "/petty_cash/view.php?id=" . $request_id, 2500, "error");
        exit;
    }
    ...
    pop(
        "Signed reconciliation form uploaded successfully! Procurement team will review it shortly.",
        "/petty_cash/view.php?id=" . $request_id,
        2500,
        "success"
    );
    ...
    error_log("Signed form upload error for petty cash request " . $request_id . ": " . $e->getMessage());
    pop(extractDbMessage($e), "/petty_cash/view.php?id=" . $request_id, 2500, "error");
}
?>
