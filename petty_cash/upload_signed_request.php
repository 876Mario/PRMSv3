<?php
/**
 * Petty Cash: Upload Signed Request Document
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
    pop('Invalid request (CSRF check failed)', '/petty_cash/list.php', 2500, 'error');
    exit;
}
...
if (!isset($_FILES['signed_request_file']) || $_FILES['signed_request_file']['error'] === UPLOAD_ERR_NO_FILE) {
    pop('No file provided', '/petty_cash/view.php?request_id=' . $request_id, 2500, 'error');
    exit;
}
...
if (!$result['success']) {
    pop($result['message'], '/petty_cash/view.php?request_id=' . $request_id, 3000, 'error');
    exit;
}
...
pop(
    'Signed petty cash form uploaded successfully (Version ' . $result['version'] . ')',
    '/petty_cash/view.php?request_id=' . $request_id,
    2500,
    'success'
);
