...
function loadPettyCashDocControlSettings(PDO $pdo): array
{
    try {
        $stmt = $pdo->prepare("SELECT * FROM doc_ctrl_settings WHERE request_type = 'PETTY_CASH' LIMIT 1");
        $stmt->execute();
...
$docCtrl = loadPettyCashDocControlSettings($pdo);
